...
if "code" in query_params and not st.session_state.get("google_login_done", False):
    ...
    try:
        ...
        if config_path.exists():
            ...
            config = {"credentials": {"usernames": {}}, "cookie": {"name": "copey_cookie", "key": "super_secret_signature_key", "expiry_days": 30}, "preauthorized": {"emails": []}}
        ...
        if username_key not in config["credentials"]["usernames"]:
            config["credentials"]["usernames"][username_key] = {
                ...
                "email": email,
                "password": safe_bcrypt_hash("google_oauth_user")  # truncate safely
            }
...
with st.form("input_form"):
    content_target = st.text_area("What part of your website do you want to change?", placeholder="Homepage headline, CTA, etc.")
    ...
    desired_outcome = st.text_area("Describe desired improvement:", placeholder="More persuasive, clearer message...")
